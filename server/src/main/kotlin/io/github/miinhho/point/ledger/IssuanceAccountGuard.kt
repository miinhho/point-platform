 ...
 *
 * 시드보다 뒤에 선다. ApplicationRunner 는 「Started PointApplicationKt」가 찍힌 뒤에
 * 돌므로, 이 가드가 세운 부팅도 로그에서는 성공 줄 다음에 실패가 온다.
 */
@Component
@Order(2)
class IssuanceAccountGuard(private val accountRepository: AccountRepository) : ApplicationRunner {
    override fun run(args: ApplicationArguments) {
        val missing = accountRepository.pointTypeIdsWithout(AccountKind.ISSUANCE)
