...
/** 계정을 여는 곳. 시드도 창설 서비스를 지나므로 실서버에서 이것을 부르는 곳은 하나다. */
@Service
class Accounts(private val accountRepository: AccountRepository) {
     ...
     *
     * 스키마로 강제하지 못한다 — 트리거는 binlog 가 켜진 MySQL 에서 SUPER 를 요구하고 그것은
     * 앱 사용자에게 줄 권한이 아니다. 대신 **포인트를 만드는 길이 이 하나뿐**이게 두고,
     * 창설이 계정을 여는지를 AccountsTest 가 본다.
     */
