...
class CapChangeService(
    ...
    fun findByIdempotencyKey(key: String, viewerId: Long): PointTypeResponse? =
        capChangeRepository.findByIdempotencyKey(key)
    ...
    fun changeCap(actorId: Long, publicId: String, idempotencyKey: String, requested: BigDecimal?): PointTypeResponse {
        val newCap = requested?.asSafeInteger()
            ?: throw DomainFailureException("SERVER", HttpStatus.BAD_REQUEST, "요청 형식 오류")
        if (newCap <= 0) throw DomainFailureException("SERVER", HttpStatus.BAD_REQUEST, "요청 형식 오류")
        ...
        capChangeLookup.freshFindByIdempotencyKey(idempotencyKey)?.let {
            return pointType.toResponse(actorId, pointTypeRepository.sharedNames())
        }

        if (pointType.issuer.id != actorId) {
            throw DomainFailureException("NOT_ISSUER", HttpStatus.FORBIDDEN, "발행자가 아님")
        }
        if (newCap < pointType.totalIssued) {
            // 그 아래로 내리면 유통량이 상한을 넘은 상태가 되어 상한이 뜻을 잃는다.
            throw DomainFailureException("CAP_BELOW_ISSUED", HttpStatus.UNPROCESSABLE_ENTITY, "이미 발행한 양보다 낮음")
        }
        if (newCap == pointType.issueCap) {
            // 이력에 남는 사건이므로 아무것도 바꾸지 않는 줄을 만들지 않는다.
            throw DomainFailureException("SERVER", HttpStatus.BAD_REQUEST, "지금과 같은 값")
        }
