...
interface CapChangeRepository : JpaRepository<CapChange, Long> {
    ...
    @Query("select c from CapChange c where c.journalEntry.requester.id = :requesterId and c.journalEntry.idempotencyKey = :key")
    ...
    @Query(
        "select c from CapChange c where c.journalEntry.pointType.id in :pointTypeIds " +
            "and (:pointTypeId is null or c.journalEntry.pointType.id = :pointTypeId) " +
