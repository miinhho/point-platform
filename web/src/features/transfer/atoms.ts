 ...
 * 보내기 플로우.
 *
 * 초안과 내비게이션을 **각자 모델로 두고 여기서 묶는다.** 1차 구현은 둘을 한
 * 리듀서에 넣었고, 그래서 이체와 무관한 내역 화면이 "이체 플로우" 안에 들어왔다.
 *
 * 이 파일의 쓰기 아톰들이 곧 "행동"이다. 화면은 `set(startTransferAtom, ...)` 만
 * 부르고, 초안을 어떻게 만들고 어느 화면으로 가는지는 여기서 한 번만 정한다.
 */
...
export interface StartInput {
  pointType: PointType
  kind?: TransferKind
  /** 홈에서 최근 대상을 눌러 바로 들어올 때. 대상 선택을 건너뛴다 */
  to?: User
}
 ...
 * 플로우 시작.
 *
 * 대상이 정해져 있으면 금액 화면으로 직행한다. 대상 선택을 건너뛰었다는 사실을
 * 따로 기억하지 않는다 — 스택에 그 화면이 없다는 것이 이미 그 정보다.
 */
 ...
 * 요청이 실패했다.
 *
 * 초안을 버리지 않는다. 받는 사람도 금액도 포인트도 그대로 남고, 재시도는 같은
 * 멱등성 키로 나간다 — 그 키가 없으면 재시도는 같은 돈을 두 번 보내는 일이다.
 */
