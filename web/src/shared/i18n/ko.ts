 ...
 * 문자열 카탈로그 (한국어).
 *
 * 화면에 문자열을 하드코딩하지 않는다. 1차 구현에는 화면과 UI 에 한국어 리터럴이
 * 106개 있었고, 그래서 두 가지가 불가능했다.
 *
 *  1. **문체를 한 번에 고칠 수 없다.** 문서를 `~다` 체로 쓰다가 그대로 UI 에 흘려서
 *     "아직 아무것도 처리되지 않았다" 같은 번역체가 앱 전체에 퍼졌다
 *  2. **문구 규칙을 테스트할 수 없다.** "발행 화면에 '잔액'이라는 말이 나오면 버그"를
 *     눈으로 찾아야 했고, 실제로 그 버그가 났다
 *
 * 문체는 **해요체**다. 그리고 짧게 쓴다 — 긴 경고는 읽히지 않고, 읽히지 않는 경고는
 * 없는 것과 같다. `copy.test.ts` 가 이 두 규칙을 지킨다.
 */
export const ko = {
  common: {
    back: '뒤로',
