...
**터미널로 보낸 지시는 사라진다.** 상대가 명령을 돌리는 중이면 안 들어가고(`agent_prompt_stalled`),
들어가도 화면 밖으로 밀리고, Orca 가 재시작되면 통째로 없어진다. 실제로 하루에 세 번 다
났다. 그래서 **일의 단위는 GitHub 이슈**다.

| 라벨 | 누가 |
|---|---|
| `backend` · `frontend` · `qa` · `review` | 그 세션이 한다 |
| `contract` | 조율이 계약을 정해야 한다 — 구현 세션이 여는 것 |
| `blocked` | 다른 이슈가 닫혀야 시작한다. 본문에 무엇을 기다리는지 적는다 |

**끌어가는 것이지 밀어 주는 것이 아니다.** 하던 일이 끝나면 스스로 본다.

```bash
gh issue list --label backend --state open        # 내 것
gh issue view <n>                                  # 완료 기준을 읽는다
gh issue comment <n> --body "시작한다"             # 둘이 겹치지 않게
```

PR 본문에 `Closes #<n>` 을 적으면 머지가 이슈를 닫는다.

**이슈 본문에 완료 기준이 있다.** 없으면 시작하지 않는다 — 조율에게 물어라(`CLAUDE.md` F5).
**계약은 이슈가 아니라 `docs/` 에 있고 이슈는 그 절을 가리킨다** — 계약 문장을 이슈에 옮겨
적으면 곧 한쪽이 낡는다.

**이슈에 적었으면 담당 세션에 보내기까지 해야 끝난다** — 보내는 법은 아래 「적고 나서 보낸다」.

## 끝났다는 것은 알림이 아니라 상태다

일의 분배만 이슈로 옮기면 **「끝났다」와 「봐 달라」와 「틀렸다」가 갈 곳을 잃는다.** 그것들이
지금까지 조율의 메시지로 흐르던 것이고, 그래서 조율이 병목이자 증폭기가 됐다.

가르는 기준은 하나다 — **확인할 수 있는 사실인가, 판단이 실린 말인가.**

| | 어떻게 |
|---|---|
| **확인할 수 있는 것** — PR 이 열렸다 · 머지됐다 · 이슈가 닫혔다 · 서버가 떴다 · `main` 이 움직였다 | **아무도 안 알린다.** 보는 쪽이 자기 차례에 본다 |
| **판단이 실린 것** — 승인 · 결함 · 계약이 틀렸다 · 못 재고 넘긴다 | **글로 남긴다.** 알림이 아니라 기록이다 |

**판단은 그것이 사는 자리에 적는다.**

| 무엇 | 어디에 |
|---|---|
| 그 PR 안에서 고칠 것 | **PR 코멘트** — 담당 세션이 자기 PR 을 본다 |
| 승인 | **PR 코멘트에 「이 문장이 승인이다」** — GitHub 이 자기 PR 승인을 막는다 |
| 그 PR 밖의 것 · 다음에 할 것 | **새 이슈 + 담당 라벨** |
| 계약이 틀렸거나 없다 | **새 이슈 + `contract`** — 조율이 `docs/` 에 쓴다 |
| 실기기가 현장에서 본 것 | `docs/FIELD.md` 에 적고, 고쳐야 할 것이면 **이슈** |

### 돌아올 때 보는 것

**한 덩이가 끝날 때마다 이 둘을 본다.** 이것이 「알림」을 대신한다.

```bash
gh pr list --state open --search "head:<내 접두사>/"   # 내 PR 에 무엇이 달렸나
gh issue list --label <내 라벨> --state open           # 내 다음 일
```

| 세션 | 접두사 | 라벨 |
|---|---|---|
| 프론트 | `head:web/` | `frontend` |
| 백엔드 | `head:server/` | `backend` |
| 실기기 | `head:qa/` | `qa` |
| 조율 | (자기 PR 이 없다 — `main` 직접) | `contract` |
| 리뷰 | **전부** — `gh pr list --state open` | `review` |

**`--author @me` 를 쓰지 않는다.** 다섯 세션이 같은 GitHub 계정으로 밀기 때문에 아무것도
좁히지 않는다. 세션을 가르는 것은 계정이 아니라 **브랜치 접두사**다(「세션마다 워크트리」).
더 나쁜 쪽은 남의 PR 이 섞여 목록을 훑고 「내 것에는 아무것도 없다」로 넘기는 것이다 —
알림을 없앤 자리에 둔 명령이라 이것이 유일한 계기다.

**조율도 같은 규칙 아래 있다.** 매 차례 `gh issue list --label contract --state open` 을
본다. 안 보면 구현 세션이 올린 것을 사용자가 대신 옮기게 되고, 그 순간 조율은 없앤 줄 알았던
메시지 버스로 돌아간다.

**본 것이 없으면 없다고 말하고 멈춘다.** 할 일이 없다고 새 일을 만들지 않는다 — 리뷰는
예외다(`docs/REVIEW.md` 「아무 일도 없을 때」).

### 이슈는 찾은 사람이 연다

**조율이 일을 다 적어 주지 않는다.** 일의 대부분은 일하다가 발견된다 — 막힌 것, 결함,
계약의 빈 칸, 다음에 해야 할 것. 그것을 조율에게 말해 조율이 이슈로 옮기면 **조율이 다시
병목이 된다.**

**찾은 사람이 그 자리에서 연다. 라벨은 담당이 정해지는 곳이다.**

| 찾은 것 | 라벨 |
|---|---|
| 내 자리에서 내가 고칠 것 | 내 라벨 — 내 대기줄에 넣는 것이다 |
| 남의 자리 것 | 그 세션의 라벨 |
| 계약이 틀렸거나 없다 · 누구 일인지 모르겠다 | **`contract`** |

**모르면 `contract` 다.** 라벨 없는 이슈는 아무도 자기 목록에서 못 보므로 조용히 죽는다.
`contract` 를 기본 대기줄로 두면 최소한 조율이 본다.

### 적고 나서 보낸다

**순서가 규칙의 전부다.**

1. **적는다** — 이슈 본문 · 이슈 코멘트 · PR 코멘트. 판단이 사는 자리에(위 표)
2. **그 내용을 그대로 담당 세션 터미널에 보낸다.** 조율을 거치지 않는다

```bash
orca terminal list --json    # 핸들은 Orca 재시작마다 바뀐다. 제목으로 찾는다
orca terminal send --terminal <handle> --text "<적은 것 + 어디에 적었는지>" --enter
```

**「#34 봐라」로 줄이지 않는다.** 받는 쪽이 이슈를 열고 코멘트를 훑어 무엇이 새것인지
찾아야 한다 — 특히 PR 코멘트가 여럿이면 그것만으로 한 덩이 일이 된다. **읽고 바로 움직일 수
있게 보낸다.**

**대신 어디에 적혀 있는지를 함께 말한다** — 「#34 본문에 적었다」 · 「PR #39 코멘트에
적었다」. 그래야 나중에 다시 찾고, 세 번째 세션이 끼어들 때 같은 것을 읽는다.

**중복이 아니다.** 적은 것이 **기록**이고 보낸 것은 그 기록의 **전달**이다. 진실은 한 곳에
있다.

**안 들어가도 재시도하지 않는다.** 상대가 명령을 돌리는 중이면 `agent_prompt_stalled` 인데
**잃는 것이 없다** — 기록이 남아 있고 상대는 자기 차례에 목록에서 만난다. 보내는 것은 그
차례를 앞당기는 것뿐이다. 기다리지도 않는다(`CLAUDE.md` 「대기는 작업을 멈추는 이유가
아니다」).

**적기만 하고 안 보내면 사용자가 대신 옮기게 된다.** 세션 사이의 통지는 **세션끼리** 한다.

**순서는 번호 순이다.** 자기 라벨의 열린 이슈를 위에서부터 하나씩 한다. 바꿔야 하면 조율이
그 이슈에 「이것 먼저」를 코멘트한다 — 라벨을 하나 더 만들지 않는다.

### 막힌 것은 막은 쪽이 푼다

`blocked` 이슈는 본문에 **`Blocked by #NN`** 을 적는다. 기다리는 쪽은 대기 중이라 아무것도
돌지 않아 스스로 알아챌 계기가 없다. 그래서 **`#NN` 을 닫는 세션이 `blocked` 를 떼고 한 줄
코멘트한다.**

```bash
gh issue list --label blocked --state open   # 머지 직후에 본다. 내 번호를 가리키는 것이 있나
gh issue edit <n> --remove-label blocked
```

빠뜨리면 그 이슈는 영원히 안 열린다. **닫는 것과 푸는 것은 한 동작이다.**
