...
**열었으면 그 세션에 한 줄 찔러 준다. 조율을 거치지 않는다.**

```bash
orca terminal list --json     # 핸들은 Orca 재시작마다 바뀐다. 제목으로 찾는다
orca terminal send --terminal <handle> --text "이슈 #NN 열었다. gh issue view NN 로 봐라" --enter
```

**한 줄만 보낸다.** 일의 내용은 이슈에 있다 — 터미널로 다시 적으면 같은 문장이 두 곳에
살고 화면이 덮인다. 상대가 명령을 돌리는 중이면 `agent_prompt_stalled` 로 안 들어가는데
**그래도 잃는 것이 없다** — 이슈가 남아 있고 상대는 자기 차례에 목록에서 만난다. 찌르는 것은
그 차례를 앞당기는 것뿐이다. 그래서 **재시도하지 않는다. 기다리지도 않는다.**

이것을 안 하면 사용자가 대신 옮기게 된다. 세션 사이의 통지는 **세션끼리** 한다.
