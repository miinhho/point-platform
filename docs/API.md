...
`docs/JOURNEY.md` 의 방향(포인트 플랫폼)이 앞으로 여기에 더할 것들이다. **아직 확정되지
않았으므로 구현하지 않는다.** 다만 스키마에 영향이 있는 것 하나는 지금 정해 둔다 —
`PointType.visibility` 다. 나중에 컬럼을 더하면 이미 만들어진 행에 값이 없고, 그때
"기존 것은 공개인가 비공개인가" 를 뒤늦게 정하게 된다. 지금 만들어지는 것은 전부
`public` 이다. 지금 동작(누구나 받을 수 있다)이 곧 공개이기 때문이다.

확정되면 더할 것: 회원 자격(가입·초대·내보내기), 게시 품목과 교환권, SDK 용 자격 증명.

**내보내기 규칙은 미리 못박아 둔다.** 비공개 은행에서만 가능하고 **포인트는 회수하지
않는다.** 회원이 아니게 된 사람의 잔액은 그대로 남고, 비공개 은행에서는 쓸 수 없는 채로
남는다. 이것은 결함이 아니라 설계다 — 잔액을 지우거나 옮기는 코드를 쓰지 않는다.
